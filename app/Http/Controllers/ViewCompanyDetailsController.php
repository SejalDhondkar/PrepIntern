...
class ViewCompanyDetailsController extends Controller
{
    public function viewDetails($id){
        ...
        $company_admin_details = User::where('id',$company_basic_info->admin_id)->first();
        $company_address = CompanyAddress::where('company_id',$company_id)->first();
        if($company_address->city_id){
            $company_address->city_name = Cities::where('id',$company_address->city_id)->value('name');
        }else{
            $company_address->city_name = '';
        }

        if($company_address->state_name){
            $company_address->state_name = States::where('id',$company_address->state_id)->value('name');
        }else{
            $company_address->state_name = '';
        }
        $company_address->country_name = Countries::where('id',$company_address->country_id)->value('name');
        ...
        'company_additional_details'=>$company_additional_details, 'company_admin_details'=>$company_admin_details,
