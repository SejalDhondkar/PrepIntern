<template>
  <v-card outlined
    ...
    <v-toolbar color="blue dark-3" dark>
      <v-card-text>
    ...
    <v-form>
      <v-container class="py-0">
        <v-row>
            ...
            <v-menu offset-y>
              ...
              <v-list v-if="data_results.length">
                <v-list-item-group v-model="data" color="primary">
                  <v-list-item v-for="(data, i) in data_results"
                               :key="i"
          ...
          <v-col cols="4" class="text-center" v-if="student.country_id == '101'">
            <v-subheader>State</v-subheader>
          </v-col>

          <v-col cols="8" v-if="student.country_id == '101'">
            <v-menu offset-y>
              <template v-slot:activator="{ on }">
                          <v-text-field
                            :disabled="state"
              ...
              <v-list v-if="state_data_results.length">
                <v-list-item-group v-model="data" color="primary">
                  <v-list-item v-for="(data, i) in state_data_results"
                               :key="i"
          ...
          <v-col cols="4" class="text-center" v-if="(student.state_id != '24' || student.state_id != '25' || student.state_id != '28') && student.country_id =='101' ">
            <v-subheader>City</v-subheader>
          </v-col>

          <v-col cols="8" v-if="(student.state_id != '24' || student.state_id != '25' || student.state_id != '28') && student.country_id =='101' ">
            <v-menu offset-y>
              <template v-slot:activator="{ on }">
                          <v-text-field
                            :disabled="city"
              ...
              <v-list v-if="city_data_results.length">
                <v-list-item-group v-model="data" color="primary">
                  <v-list-item v-for="(data, i) in city_data_results"
                               :key="i"
    ...
    methods: {
      ...
      autoCompleteState() {
        ...
        if (this.statesearchquery.length > 1) {
          axios
            .get('/company/address/statesearch', {
              params: { statesearchquery: this.statesearchquery, country_id: this.student.country_id }
            })
            .then(response => {
      ...
      autoCompleteCity() {
        ...
        if (this.citysearchquery.length > 2) {
          axios
            .get('/company/address/citysearch', {
              params: { citysearchquery: this.citysearchquery, state_id: this.student.state_id }
            })
            .then(response => {
