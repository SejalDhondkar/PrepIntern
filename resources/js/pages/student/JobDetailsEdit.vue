<template>
  ...
  >
    ...
    >
    <v-card-text>
      <v-toolbar-title class="text-center">Job Details</v-toolbar-title>
...
<script>
  export default {
    ...
    created(){
		...
		this.axios.get(`/student/jobdetails/${this.$route.params.id}/edit`).then((response) => {
  ...
  methods: {
    ...
    submit() {
      this.errors = {};
      axios.post(`/student/jobdetails/${this.$route.params.id}/update`, this.student).then(response => {
        this.$router.go(-1);
      }).catch(error => {
