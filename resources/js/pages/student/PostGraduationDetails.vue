<template>
  <v-card outlined
    ...
    <v-toolbar color="purple darken-1" dark>
      <v-card-text>
        <v-toolbar-title class="text-center">Post Graduation Details</v-toolbar-title>
    ...
    <v-form>
      <v-container class="p-4">
        ...
        <p class="font-weight-medium">Post Graduation status:</p>
...
<script>
  export default {
    ...
    created() {
      ...
      this.axios.get('/student/postgraduationdetails/edit').then(response => {
    ...
    methods: {
      ...
      autoCompleteCollege() {
        ...
        if (this.collegesearchquery.length > 2) {
          axios
            .get('/student/collegesearch', { params: { collegesearchquery: this.collegesearchquery } })
            .then(response => {
